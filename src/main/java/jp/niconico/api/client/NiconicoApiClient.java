...
public class NiconicoApiClient {
	...
	public boolean isLogin() {
		boolean ret = true;
		if (loginInfo == null || StringUtils.isBlank(loginInfo.mail)
	 ...
	 *            検索クエリ
	 * @param sort
	 *            ソート種別 n:コメント日時 v:再生数 m:マイリスト r:コメント数 f:投稿日時 l:再生時間
	 * @param page
	 *            取得ページ
	 * @param order
	 *            d:降順 a:昇順
	 * @param tagSearch
	 *            false:キーワード検索 true:タグ検索
	 * @return 検索結果
	 * @throws NiconicoException
	 */
	 ...
	 *            動画ID
	 * @param date
	 *            時刻。ここで指定した時点で表示されるコメントが取れる
	 * @return コメント
	 * @throws NiconicoException
	 */
	public List<CommentInfo> getPastComment(String id, Date date)
			throws NiconicoException {
		FlvInfo flvInfo = getFlv(id);
	 ...
	 * 
	 * @param period
	 *            期間 hourly:1時間 daily:1日 weeky:週間 monthly:月間 total:全ての期間
	 * @param rankKind
	 *            ランキング種別 fav:総合 view:再生数 res:コメント数 mylist:マイリスト数
	 * @return ランキング情報
	 * @throws NiconicoException
	 */
	public List<RankingInfo> getRanking(RankingPeriod period,
			RankingKind rankKind) throws NiconicoException {
	 ...
	 * @throws NiconicoException
	 */
	public List<MylistItem> getMylistItems(String mylistId)
			throws NiconicoException {
		NicoGetMylist method = new NicoGetMylist(loginInfo);
	 ...
	 * @throws NiconicoException
	 */
	public File downloadVideo(String id, String destDir)
			throws NiconicoException {
		FlvInfo flvInfo = getFlv(id);
		ThumbInfo thumbInfo = getThumbInfo(id);
		NicoDownloadVideo method = new NicoDownloadVideo(id, loginInfo,
				flvInfo, thumbInfo);
